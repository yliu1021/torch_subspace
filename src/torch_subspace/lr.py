...
if __name__ == "__main__":
    ...
    x = SubspaceLR(num_rows=10, num_cols=8)
    print("Initial effective weights:")
    init_eff_weights = x.eff_weights().detach()
    print(init_eff_weights)

    print("Setting blocks")
    x.set_blocks(row_sizes=[4, 6], col_sizes=[3, 5])
    set_block_eff_weights = x.eff_weights()
    print("Eff weight diff:")
    print(set_block_eff_weights - init_eff_weights)

    x.set_mask(0, 0, torch.tensor([1, 1, 0]))
    svd_eff_weights = x.eff_weights()
    print("SVD eff weights diff:")
    print(svd_eff_weights - init_eff_weights)

    x.clear_mask(0, 0)
    clear_svd_eff_weights = x.eff_weights()
    print("Cleared SVD eff weights diff:")
    print(clear_svd_eff_weights - init_eff_weights)

    x.set_mask(0, 0, torch.tensor([1, 0, 0]))
    print(x.eff_numel())
    svd_eff_weights = x.eff_weights()
    print("re-SVD eff weights diff:")
    print(svd_eff_weights - init_eff_weights)

    # Make sure we can still perform backprop
    ###########################
    ...
    opt = SGD(x.parameters(), lr=0.1)
    for _ in range(1000):
        loss = torch.norm((x.eff_weights() - init_eff_weights), p=torch.inf)
        opt.zero_grad()
        loss.backward()
        opt.step()
    print(x.eff_weights() - init_eff_weights)
